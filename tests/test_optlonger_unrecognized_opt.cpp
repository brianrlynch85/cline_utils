// -----------------------------------------------------------------------
//
//                      test_optlonger_unrecognized_opt.cpp V 0.01
//
...
* 
*/
TEST_CASE("UNRECOGNIZED OPTION","[MUSTFAIL]")
{
   //std::cout << "-- BEGIN Unrecognized Option Test --" << std::endl;
   ...
   REQUIRE_THROWS_WITH(_G_cline->parse_command_line(), Catch::Matchers::ContainsSubstring(" - Unrecognized option"));
   _G_cline->print_input_summary();
   _G_cline->delete_all_options();

   //std::cout << "-- End Unrecognized Option Test --" << std::endl;
}
