// -----------------------------------------------------------------------
//
//                      test_optlonger_missing_opt.cpp V 0.01
//
...
* 
*/
TEST_CASE("MISSING OPTION","[MUSTFAIL]")
{
   //std::cout << "-- BEGIN Missing Option Test --" << std::endl;
   ...
   REQUIRE_THROWS_WITH(_G_cline->parse_command_line(), Catch::Matchers::ContainsSubstring(" - Missing required option"));
   _G_cline->print_input_summary();
   _G_cline->delete_all_options();

   //std::cout << "-- End Missing Option Test --" << std::endl;
}
